...
import { X, ShieldCheck, ShieldOff, LogOut, Mail, CheckCircle2, AlertCircle } from "lucide-react";
import { toast } from "sonner";
import { supabase } from "@/integrations/supabase/client";

export function SettingsSheet({ onClose }: { onClose: () => void }) {
  const { user, hasPin, setPin, removePin, signOut, displayName } = useAuth();
  const [mode, setMode] = useState<"menu" | "set" | "remove">("menu");
